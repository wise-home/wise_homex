defmodule WiseHomex.ApiClientTest do
  ...
  describe "vacancy info" do
    test "it deletes a vacancy info", %{config: config} do
      ...
      {:ok, _vacancy_info} = config |> WiseHomex.delete_vacancy_info("0a482be2-25c0-4449-9e1b-80f8256c94f9")

      assert MockServer.called?(:delete_vacancy_info) == %{id: "0a482be2-25c0-4449-9e1b-80f8256c94f9"}
    end
  end
  ...
  test "unlink external entities happy path", %{config: config} do
    MockServer.set(
    ...
    assert {:ok, :empty} =
             WiseHomex.property_unlink_external(
               config,
               "property_id",
               %{does_not_matter: :does_not_matter}
             )

    assert MockServer.called?(:property_unlink_external) == %{
             type: "unlink-external",
             id: "property_id",
             attrs: %{does_not_matter: :does_not_matter}
           }
